          ...
          <div class="mvp-matches">
            ${mvp.matchesInfo.map(info => {
              ...
              <div class="mvp-match-badge ${isExact ? 'mvp-match-exact' : 'mvp-match-normal'}" onclick="event.stopPropagation(); window.history.pushState(null, null, '/perfil/${mvp.id}?hl=${info.match.id}'); window.router();" style="cursor: pointer; position: relative; z-index: 2;">
                <span style="color: white; font-weight: 600;">${teams[info.match.homeTeam]?.flag || ''} ${teams[info.match.homeTeam]?.codeEsp || info.match.homeTeam} ${info.prediction ? `${info.prediction.home}-${info.prediction.away}` : '-'} ${teams[info.match.awayTeam]?.codeEsp || info.match.awayTeam} ${teams[info.match.awayTeam]?.flag || ''}</span>
                <span style="color: ${isExact ? '#ffd700' : (info.points > 0 ? '#4cd137' : 'var(--text-secondary)')}; font-weight: bold;">+${info.points}</span>
              </div>
              `;
            }).join('')}
          </div>

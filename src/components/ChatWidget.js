      ...
      return `
        ...
        <div class="chat-message ${isMe ? 'chat-message-me' : 'chat-message-other'}" id="msg-${msg.id}">
          ...
          <div class="chat-bubble-content">
            ...
            ${msg.replyTo ? `
              <div class="chat-replied-msg" ${msg.replyTo.isMVP ? `onclick="window.closeChat(); window.history.pushState(null, null, '/perfil/${msg.replyTo.uid}'); window.router();"` : (!msg.replyTo.isPrediction ? `onclick="document.getElementById('msg-${msg.replyTo.id}')?.scrollIntoView({behavior: 'smooth', block: 'center'})"` : `onclick="window.closeChat(); window.history.pushState(null, null, '/perfil/${msg.replyTo.uid}?hl=${msg.replyTo.matchId}'); window.router();"`)} style="cursor: pointer;">
                <div class="reply-name">${msg.replyTo.isMVP ? '👑 MVP de la Jornada' : (msg.replyTo.isPrediction ? '⚽ Pronóstico de ' + msg.replyTo.name : msg.replyTo.name)}</div>
                <div class="reply-text">${msg.replyTo.isMVP ? `<strong>${msg.replyTo.name}</strong> sumó ${msg.replyTo.points} pts` : (msg.replyTo.isPrediction ? `<strong>${msg.replyTo.matchStr}</strong>: ${msg.replyTo.predStr}` : (msg.replyTo.text || 'GIF'))}</div>
              </div>
            ` : ''}
        ...
        ${replyingTo ? `
          <div class="chat-reply-preview">
            <div style="flex: 1; overflow: hidden; margin-right: 0.5rem;">
              <div class="reply-name">${replyingTo.isMVP ? 'MVP de la Jornada' : (replyingTo.isPrediction ? 'Compartiendo pronóstico de ' + replyingTo.name : 'Respondiendo a ' + replyingTo.name)}</div>
              <div class="reply-text">${replyingTo.isMVP ? `${replyingTo.name} sumó ${replyingTo.points} pts` : (replyingTo.isPrediction ? `${replyingTo.matchStr}: ${replyingTo.predStr}` : replyingTo.text)}</div>
            </div>
